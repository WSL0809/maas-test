...
>   - suppress reasoning：依据 `tests/test_chat.py` 中 `test_create_suppresses_reasoning_when_thinking_disabled` 与 `test_stream_suppresses_reasoning_when_thinking_disabled` 回填；`qwen35` 与 `glm-5` 稳定通过，`kimi-k25`、`minimax-m21`、`minimax-m2.5` 若仍返回 `reasoning` 则以 `xfail` 记录，因此整体仍按 `⚠️` 记为部分通过。
...
| # | 测试点 | 说明 |
|---|--------|------|
| M-1 | `enable_thinking=false` 下的 `reasoning` 字段 | 当前已知请求可接受，但 `reasoning` 仍可能返回文本，需要比较 `minimax-m21` 与 `minimax-m2.5` |
