...
- 定向 live 复测结果：`glm-5`、`minimax-m21`、`minimax-m25` 通过；`qwen35` 与 `kimi-k25` 当前不稳定。
...
- B8/B9 定向复测（`tests/test_chat.py`）：`7 passed, 3 xfailed`；B9（StructuredOutput）全模型通过，B8 在 `glm-5`/`minimax-m21`/`minimax-m25` 出现 JSON 落在 `message.reasoning` 的通道差异并以 `xfail` 记录。
