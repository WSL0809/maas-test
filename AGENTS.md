...
- [test_chat.py](/Users/wangshilong/Downloads/maas-test/test_chat.py): the default `httpx`-based pytest suite, organized as shared base tests plus per-model subclasses for model-specific request/response expectations.
- [test_chat_strict.py](/Users/wangshilong/Downloads/maas-test/test_chat_strict.py): strict OpenAI-compatible audit suite. Default skip; enable explicitly when you want to inspect compatibility gaps.
...
- `uv run pytest -q test_chat.py`: run the default per-model optimized live chat suite.
- `uv run pytest -q test_chat.py --chat-model glm5 --chat-model qwen35 --chat-model minimax-m25 --chat-model minimax-m21 --chat-model kimi-k25`: run the optimized suite against a chosen model matrix.
- `uv run pytest -q test_chat_strict.py --run-strict-compat`: run the strict OpenAI-compatible audit suite.
- `uv run pytest -q test_chat_sdk_smoke.py --run-sdk-smoke`: run the OpenAI SDK smoke suite.
- `uv run pytest -q --run-strict-compat --run-sdk-smoke`: run everything, including strict audit and SDK smoke.
...
- Keep strict compatibility assertions in `test_chat_strict.py` rather than reintroducing large `if/else` branches in the default suite.
