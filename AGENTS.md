...
- `uv run pytest -q tests/test_chat.py tests/test_api_compatibility.py tests/test_context_length.py tests/test_tool_calling.py --chat-model glm-5 --chat-model qwen35 --chat-model minimax-m2.5 --chat-model minimax-m21 --chat-model kimi-k25`: run the optimized suite against a chosen model matrix.
